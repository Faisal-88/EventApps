...
if (isset($_POST['add_event'])) {
    ...
    $query = mysqli_query($connection, "INSERT INTO events (event_name, event_description, event_date) VALUES ('$eventName', '$eventDescription', '$eventDate')");

    if ($query) {
        $notif = '<div class="alert alert-success">Berhasil tambah data</div>';
    } else {
        $notif = '<div class="alert alert-danger">Gagal tambah data: ' . mysqli_error($connection) . '</div>';
    }
}

if (isset($_POST['add_user_event'])) {
    $fullname = mysqli_real_escape_string($connection, $_POST['fullname']);
    $email = mysqli_real_escape_string($connection, $_POST['email']);

    // Ambil id_user dari sesi atau form, sesuai dengan kebutuhan aplikasi Anda
    $idUserValue = 123; // Ganti dengan nilai yang sesuai

    // Ambil id_event dari tabel events
    $queryEvents = mysqli_query($connection, 'SELECT * FROM events');
    $dataEvent = mysqli_fetch_array($queryEvents);
    $idEventValue = $dataEvent['id_event'];

    $queryInsert = mysqli_query($connection, "INSERT INTO user_event (id_user, id_event, fullname, email) VALUES ('$idUserValue', '$idEventValue', '$fullname', '$email')");

    if ($queryInsert) {
        $notif = '<div class="alert alert-success">Berhasil tambah data user_event</div>';
    } else {
        $notif = '<div class="alert alert-danger">Gagal tambah data user_event: ' . mysqli_error($connection) . '</div>';
    }
}
...
if (isset($_GET['delete'])) {
    ...
    if ($query) {
        ...
        $notifDelete = '<div class="alert alert-danger">Gagal hapus data: ' . mysqli_error($connection) . '</div>';
    }
}
?>
                ...
                <div class="card border-0 shadow-lg">
                    <div class="card-body">
                        ...
                        <ul class="list-group">
                            ...
                            while ($data = mysqli_fetch_array($query)) {
                            ?>
                                <li class="list-group-item"><span><?= $data['event_name'] ?></span>/ <?php echo $data['fullname'] ?>/</php echo $data['email'] ?></li>                                                            <?php } ?>
                                                        </ul>
                                                    </div>
                                                </div>
                                            </div>
